...
export const TodoItem = ({ id, isCompleted, text }: Todo) => {
  ...
  const handleUpdate = (todoId: string) => {
    if (editingTodoText.trim() !== '') {
      editTodo(todoId, editingTodoText.trim())
  ...
  return (
    <motion.li
      ...
      className={cn(
        ...
        isCompleted && 'bg-opacity-50 text-zinc-500',
      )}
    >
      {editingTodoId === id ? (
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex justify-between gap-5 text-white">
            <button onClick={() => handleStatusUpdate(id)}>
              {isCompleted ? (
